 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quiz_inagradesuggestion_table extends quiz_attempts_report_table {
    ...
    protected function submit_buttons() {
        if (has_capability('mod/quiz:regrade', $this->context)) {
            echo '<input type="submit" name="regrade" value="' .
                    get_string('regradeselected', 'quiz_inagradesuggestion') . '"/>';
        }
    ...
    public function col_regraded($attempt) {
        if ($attempt->regraded == '') {
            ...
            return get_string('needed', 'quiz_inagradesuggestion');
        } else if ($attempt->regraded == 1) {
            return get_string('done', 'quiz_inagradesuggestion');
        }
    }
    ...
    protected function get_regraded_questions() {
        ...
        $regradedqs = $DB->get_records_select('quiz_inagradesuggestion_regrades',
