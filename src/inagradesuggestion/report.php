 ...
 * Extended from Martin Dougiamas' quiz overview code (1999).
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quiz_inagradesuggestion_report extends quiz_attempts_report {

    public function display($quiz, $cm, $course) {
        global $CFG, $DB, $OUTPUT, $PAGE;

        list($currentgroup, $students, $groupstudents, $allowed) =
                $this->init('inagradesuggestion', 'quiz_inagradesuggestion_settings_form', $quiz, $cm, $course);
        $options = new quiz_overview_options('inagradesuggestion', $quiz, $cm, $course);
        ...
        $filename = quiz_report_download_filename(get_string('overviewfilename', 'quiz_inagradesuggestion'),
        ...
        if ($hasquestions && ($hasstudents || $options->attempts == self::ALL_WITH)) {
                ...
                if (has_capability('mod/quiz:regrade', $this->context)) {
                    ...
                    if ($currentgroup) {
                        ...
                        $regradealldrydolabel =
                                get_string('regradealldrydogroup', 'quiz_inagradesuggestion', $a);
                        $regradealldrylabel =
                                get_string('regradealldrygroup', 'quiz_inagradesuggestion', $a);
                        $regradealllabel =
                                get_string('regradeallgroup', 'quiz_inagradesuggestion', $a);
                    } else {
                        $regradealldrydolabel =
                                get_string('regradealldrydo', 'quiz_inagradesuggestion', $regradesneeded);
                        $regradealldrylabel =
                                get_string('regradealldry', 'quiz_inagradesuggestion');
                        $regradealllabel =
                                get_string('regradeall', 'quiz_inagradesuggestion');
                    }
            ...
            if (!$table->is_downloading() && has_capability('mod/quiz:regrade', $this->context) &&
                ...
                $columns[] = 'regraded';
                $headers[] = get_string('regrade', 'quiz_inagradesuggestion');
            }
        ...
        if (!$table->is_downloading() && $options->usercanseegrades) {
            ...
            if ($currentgroup && $groupstudents) {
                ...
                if ($DB->record_exists_select('quiz_grades', "userid $usql AND quiz = ?",
                        $params)) {
                    $imageurl = new moodle_url('/mod/quiz/report/inagradesuggestion/overviewgraph.php',
                            array('id' => $quiz->id, 'groupid' => $currentgroup));
                    $graphname = get_string('overviewreportgraphgroup', 'quiz_inagradesuggestion',
            ...
            if ($DB->record_exists('quiz_grades', array('quiz'=> $quiz->id))) {
                $imageurl = new moodle_url('/mod/quiz/report/overview/overviewgraph.php',
                        array('id' => $quiz->id));
                $graphname = get_string('overviewreportgraph', 'quiz_inagradesuggestion');
                echo $output->graph($imageurl, $graphname);
            }
        }
        return true;
    ...
    protected function finish_regrade($nexturl) {
        redirect($nexturl, get_string('regradecomplete', 'quiz_inagradesuggestion'), null, \core\output\notification::NOTIFY_SUCCESS);
    }
    ...
    protected function regrade_attempts_needing_it($quiz, $groupstudents) {
        ...
        foreach ($attempts as $attempt) {
            ...
            $a['done']++;
            $progressbar->update($a['done'], $a['count'],
                    get_string('regradingattemptxofy', 'quiz_inagradesuggestion', $a));
        }
